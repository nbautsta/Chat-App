...
export default function Chat(){
  ...
  return (
    <div className="bg-no-repeat bg-cover bg-center flex h-screen" style={{ backgroundImage: "url('/images/bg.jpg')" }}>
      <div className="bg-black bg-opacity-75 backdrop-filter backdrop-blur-md w-1/3 p-4 text-white pl-4 pt-4 flex flex-col">
        ...
        <div className="p-2 text-center flex items-center justify-center">
          <span className="mr-2 text-sm text-white flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 pr-1">
          ...
          <button
            onClick={logout}
            className="text-xs bg-purple-800 pl-2 pr-2 pt-1 pb-1 text-white ml-2 rounded-full">Logout</button>
        </div>
      </div>
            ...
            <div className="relative h-full">
              ...
              {messagesWithoutDupes.map(message => (
                <div key={message._id} className={(message.sender === id ? 'text-right': 'text-left')}>
                  <div className={"text-left inline-block p-2 my-2 rounded-full text-sm " +(message.sender === id ? 'bg-purple-800 text-white':'bg-black bg-opacity-50 text-white')}>
                    {message.text}
                    {message.file && (
                        <div className="">
